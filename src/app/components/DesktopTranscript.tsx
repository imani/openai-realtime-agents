...
export interface DesktopTranscriptProps {
  userText: string;
...
function DesktopTranscript({
  userText,
  setUserText,
  onSendMessage,
  canSend,
  downloadRecording,
}: DesktopTranscriptProps) {
  ...
  function scrollToBottom() {
    if (desktopTranscriptRef.current) {
      desktopTranscriptRef.current.scrollTop =
        desktopTranscriptRef.current.scrollHeight;
    }
  }
  ...
  const handleCopyDesktopTranscript = async () => {
    if (!desktopTranscriptRef.current) return;
    try {
      await navigator.clipboard.writeText(
        desktopTranscriptRef.current.innerText
      );
      setJustCopied(true);
...
export default DesktopTranscript;
